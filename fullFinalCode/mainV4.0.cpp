...
// PID Variables
double pidSetpoint = targetDepth;  // Desired depth
double pidInput = 0;               // Current depth
double pidOutput = 0;              // PWM output
// for debug
...
void setup() {
  ...
  depthPID.SetOutputLimits(0, 255);  // Full range of PWM
...
void reelOperation() {
  ...
  {
    pidInput = depth;                    // Feedback
    pidSetpoint = targetDepth;          // Desired depth
    depthPID.Compute();                 // Update pidOutput

    if (depth < targetDepth - 0.02) {   // Allow small deadzone
      digitalWrite(motorDirPin, HIGH);  // Reel down
      analogWrite(motorPWMPin, (int)pidOutput);
      reelingDown = true;
    } 
    else if (depth > targetDepth + 0.02) { // Overshot
      digitalWrite(motorDirPin, LOW);   // Reel up slightly
      analogWrite(motorPWMPin, (int)pidOutput);
      reelingUp = true;
    } 
    else {
  ...
  if (sensorReadingsComplete == true) {
