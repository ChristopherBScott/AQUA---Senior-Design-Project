...
const unsigned long deployedWaitTime = 90000;  //was 90000
...
const float V0 = 0.06;
const float V100 = 0.95;
const float calibTempC = 21.1;
float voltage = 0;
...
const float lowSpeedThreshold = 0.3;
...
const float targetDepth = .5;   //change based on desired depth
...
void reelOperation() {
  ...
  if (waypointReached) {
    // Stop drop immediately if ultrasonic sensor says we're too close to the bottom
    ...
    if (abs(depth - pidSetpoint) < pidSetpoint * 0.05) {
      if (withinRangeStart == 0) {
        withinRangeStart = millis();
      } else if (millis() - withinRangeStart >= 5000) {
        analogWrite(motorPWMPin, 0);
        targetDepthReached = true;
        waypointReached = false;
        withinRangeStart = 0;
      ...
      withinRangeStart = 0;
      int pwm = max((int)abs(pidOutput), 22);
      if (pidOutput > 0) {
        digitalWrite(motorDirPin, HIGH); // Reel down
        analogWrite(motorPWMPin, pwm);
        reelingDown = true;
      } else {
        digitalWrite(motorDirPin, LOW);  // Reel up (safety)
        analogWrite(motorPWMPin, pwm);
      }
    }
  }
