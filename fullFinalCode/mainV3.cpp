...
// Notes: 
      V3: All sensors except for ultrasonic are connected directly to TrinketM0 microcontroller
...
void loop() {
  while(gps.getFixType() == 0) {  //Wait for GPS to initalize
    //Do nothing
    Serial.print("Waiting for GPS to Initalize...");
    delay(1000);
  }
  ...
  if (gpsSpeed > movementTrigger) { //if speed is > .5 m/s, boat is in motion, hasBeenMoving is set
    hasBeenMoving = true;
  }
  if (gpsSpeed < speedThreshold && hasBeenMoving) { // If the speed is now lower than speedThreshold(.1m/s) and was moving above 0.5 m/s, waypoint might be reached
...
void reelOperation() {
  if(waypointReached == true && depth < targetDepth - slowDownMargin) {
    digitalWrite(motorDirPin, HIGH); // Reel down
    analogWrite(motorPWMPin, fullSpeedPWM);
  }
  else if(waypointReached == true && (depth >= (targetDepth - slowDownMargin)) && (depth < targetDepth) ) {  //Slow motor down if between slowDownMargin and targetDepth
    digitalWrite(motorDirPin, HIGH); // Reel down slowly
    analogWrite(motorPWMPin, slowSpeedPWM);
  }
  else if ((waypointReached == true && depth >= targetDepth) || distance <= ultrasonicThreshold ){  //Stop motor if beyond target depth or < 100 mm from bottem
...
void logGPSData() {
  ...
  if (gpsLogFile) {
    ...
    if (hour < 10) gpsLogFile.print("0"); gpsLogFile.print(hour); gpsLogFile.print(":");
    if (minute < 10) gpsLogFile.print("0"); gpsLogFile.print(minute); gpsLogFile.print(":");
    if (second < 10) gpsLogFile.print("0"); gpsLogFile.print(second);
    gpsLogFile.print("\",\"date\":\"");
    if (month < 10) gpsLogFile.print("0"); gpsLogFile.print(month); gpsLogFile.print("-");
    if (day < 10) gpsLogFile.print("0"); gpsLogFile.print(day); gpsLogFile.print("-");
...
void manualMotorOveride() {   //Manual motor overide for when reel gets in a bad situation
